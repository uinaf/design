...
- All of `dist/` is generated and untracked, so `scripts/check.ts` must run **after** the build, never before. `verify` had them the other way round: the gate asserted on the previous build's artifacts while the current source went unchecked, and it passed only because `dist/` was committed. `check.ts` now fails by name when `dist/` is absent, and `verify` reaches it through `guide:sync`.
